...
# Convert memory strings like 16G/8192M/4096 to MB (integer)
parse_htcondor_memory() {
    local m="$1"
    m=$(echo "$m" | tr -d '[:space:]') # Strip any rogue whitespace
    if [[ "$m" =~ ^([0-9]+)[Gg]$ ]]; then
...
# Convert TIME (HH:MM:SS or HH:MM) to seconds for +MaxRuntime
time_to_seconds() {
    local t="$1"
...
read_job_state() {
    ...
    case "$status" in
        ...
        2) # Running
            ...
            return 2 ;;
        5) # Held
            print_warn "Job ${YELLOW}$JOB_ID${NC} is HELD. (Check: condor_q -analyze $JOB_ID)"
            return 3 ;;
    esac
