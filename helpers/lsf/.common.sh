...
require_writable() {
    check_writable "$1" || {
        ...
        print_info "Please run ${YELLOW}bjobs -u $USER${NC} to find jobs that might be using this image."
        exit 1
    }
}
...
#   Sources state file and checks LSF job status.
...
read_job_state() {
    ...
    # bjobs returns non-zero when job not found
...
# wait_for_job <job_id>
#   Polls bjobs until job is RUNNING. Sets global NODE variable.
#   Exits if job disappears from queue. When the job starts, attempts to locate the LSF
