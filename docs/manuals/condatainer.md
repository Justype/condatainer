...
* `--no-prebuilt`: Build from the recipe even when a registry publishes a prebuilt artifact for it. Without it, a matching prebuilt is pulled instead of built: a pulled overlay's build dependencies are not installed (install them with their own `create`); an overlay that has to be built installs them first.
