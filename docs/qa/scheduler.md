...
An **HPC cluster** is a large collection of compute machines managed as a single system. You do not run heavy work on the machine you log in to (the **login node**, which everyone shares). Instead you describe the resources your job needs and submit it to a **scheduler** — SLURM, PBS, LSF, or HTCondor — which queues the request and dispatches it to one or more **compute nodes** once those resources are free.

A job is therefore a *resource request*, and sizing it correctly is up to you.

### Threads and MPI

**Most jobs need one machine and several threads.** A typical Python, R, or bash script is a single program. It can often go faster by using more CPU cores — via NumPy, BLAS, or a `--threads` flag — but those cores are all threads sharing one machine's memory.
