...
#[derive(Parser, Debug)]
pub struct ExecArgs {
    #[arg(short = 'o', long = "overlay", help = "Overlay file to mount (can be used multiple times)")]
    pub overlays: Vec<String>,
    
    #[arg(short = 'k', long = "keep", help = "Disable automatic command parsing to installed overlays")]
    pub keep: bool,
    
    #[arg(short = 'w', long = "writable-img", help = "Mount .img overlays as writable (default: read-only)")]
    pub writable_img: bool,
    
    #[arg(long = "env", help = "Set environment variable 'KEY=VALUE' (can be used multiple times)")]
    pub env_settings: Vec<String>,
    
    #[arg(short = 'b', long = "base-image", help = "Base image to use instead of default")]
    pub base_image: Option<String>,
    
    #[arg(long = "bind", help = "Bind path 'HOST:CONTAINER' (can be used multiple times)")]
    pub bind_paths: Vec<String>,
    
    #[arg(short = 'f', long = "fakeroot", help = "Run container with fakeroot privileges")]
    pub fakeroot: bool,
    
    #[arg(trailing_var_arg = true, allow_hyphen_values = true, help = "Command to run inside the container (default: bash)")]
    pub commands: Vec<String>,
}

pub fn handle_exec(args: &ExecArgs, config: &AppConfig) -> Result<()> {
    ...
    // Parse commands to separate overlays and actual command
    let (overlay_list, command_list) = parse_commands_and_overlays(
        &args.commands,
        &args.overlays,
        &installed_overlays,
        args.keep,
    ...
    let overlay_paths = resolve_overlay_paths(&overlay_list, &installed_overlays, config)?;
    ...
    let has_writable_img = args.writable_img && ordered_overlays.iter().any(|p| is_img(&p.to_string_lossy()));
    ...
    for env_setting in &args.env_settings {
        if env_setting.contains('=') {
            env_vars.push(env_setting.clone());
        } else {
            print_warning(&format!(
                "Invalid env setting {}. It should be in KEY=VALUE format. Skipping.",
                env_setting
            ));
        }
    }
    ...
    for overlay in &ordered_overlays {
        ...
        if args.writable_img && is_img(&overlay_str) {
    ...
    // Add bind paths
    if !args.bind_paths.is_empty() {
        opts = opts.binds(args.bind_paths.clone());
    }
    ...
    // Execute
    let result = apptainer.exec(&base_image, &command_list, Some(opts));
...
/// Parse commands to separate overlays and actual command
fn parse_commands_and_overlays(
    commands: &[String],
    overlay_args: &[String],
    installed_overlays: &HashMap<String, PathBuf>,
    keep: bool,
) -> Result<(Vec<String>, Vec<String>)> {
    let mut overlay_list = Vec::new();
    let mut command_list = Vec::new();
    
    if !keep && overlay_args.is_empty() && !commands.is_empty() {
        print_debug("[EXEC] Parsing commands to separate overlays and command...");
        
        for cmd in commands {
            if is_overlay(cmd) {
                overlay_list.push(cmd.clone());
            } else {
                let normalized = normalize_name_version(cmd);
                if installed_overlays.contains_key(&normalized) {
                    print_warning(&format!(
                        "Convert command {} to overlay",
                        style_highlight(cmd)
                    ));
                    overlay_list.push(cmd.clone());
                } else {
        ...
        command_list = commands.to_vec();
    }
    
    // Add explicit overlay arguments
    overlay_list.extend_from_slice(overlay_args);
    
    // Default to bash if no command
    if command_list.is_empty() {
        command_list.push("bash".to_string());
    }
    
    Ok((overlay_list, command_list))
}
