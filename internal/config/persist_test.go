...
// Every cell is pinned: this decides the codec on a machine nobody configured,
// and picking zstd for a runtime that cannot mount it produces unreadable images.
func TestAutoDetectCompression(t *testing.T) {
	tests := []struct {
		...
		{"apptainer with zstd", true, false, "zstd-medium"},
		{"apptainer too old for zstd", false, false, "lz4"},
		{"singularity never gets zstd", true, true, "lz4"},
		{"singularity without zstd", false, true, "lz4"},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			Global.Build.CompressArgs = ""
			AutoDetectCompression(tt.supportsZstd, tt.isSingularity)
			if got, want := Global.Build.CompressArgs, ArgsForCompress(tt.want); got != want {
				t.Errorf("CompressArgs = %q, want %s (%q)", got, tt.want, want)
			}
		})
	}
}

// gzip stays reachable as an explicit choice; nothing may select it implicitly.
func TestAutoDetectCompressionNeverPicksGzip(t *testing.T) {
	for _, zstd := range []bool{true, false} {
		for _, sing := range []bool{true, false} {
			Global.Build.CompressArgs = ""
			AutoDetectCompression(zstd, sing)
			if Global.Build.CompressArgs == ArgsForCompress("gzip") {
				t.Errorf("auto-detected gzip for zstd=%v singularity=%v", zstd, sing)
			}
		}
	}
}
