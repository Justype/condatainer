...
Standing in a project (the launch's working directory holds `cnt-lock/`), each name resolves as `exec -o` does — through its pin if the project has one, else against what is installed, with a bare name expanded under the project's selected distro. A pinned artifact that is absent here refuses naming `project restore`; a name nothing installed answers is built as it is outside a project, and a helper never adds a pin. Outside a project, each name — bare, carrying a partial version, or missing its distro prefix — is resolved against what's already installed first, no network call, then built via `condatainer create <name>` (passed exactly as declared) only once nothing local satisfies it. A name that resolved to something other than itself is printed once, e.g. `rstudio-server -> ubuntu24/rstudio-server/2026.09.0-174`.
